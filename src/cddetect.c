...
static int get_drive_type(int fd)
{
	int cdcap = ioctl(fd,CDROM_GET_CAPABILITY,0);
	if (cdcap >= 0)
	{
		if (cdcap & CDC_DVD) {
			if ((cdcap & CDC_DVD_R) || (cdcap & CDC_DVD_RAM)) {
				return DRIVE_TYPE_DVDBURNER ;
			} else {
				return DRIVE_TYPE_DVDROM;
			}
		} else if (cdcap & CDC_CD_R) {
			if (cdcap & CDC_CD_RW) {
